...
def cmdline_args():
    ...
    group1.add_argument(
        "-V", "--version", help="display the program's version", action="store_true"
    )
...
class StravaConnector:
    ...
    def get_detailed_activity(self, activity_dict: Dict):
        """
    ...
    def get_gear(self, gear_id: str) -> Dict:
        ...
        r = self.client.get(self.base_url + f"/gear/{gear_id}")
        custom_raise_for_status(r)
    ...
    def filter_response_by_key(
        self, response: List[Dict], type_key: str, null_return: Any
    ):
        if not response:
            return null_return
    ...
    def create_activity_from_strava(self, activity: dict, get_streams: bool = True):
        ...
        if get_streams:
            ...
            distance = self.filter_response_by_key(
                r.json(), "distance", [0.0, activity["distance"]]
            )
            ...
            custom_raise_for_status(r)
            time_list = self.filter_response_by_key(
                r.json(), "time", [0.0, activity["moving_time"]]
            )
...
class Activity:
    ...
    def as_gpx(self) -> gpxpy.gpx.GPX:
        ...
        gpx_track = gpxpy.gpx.GPXTrack(name=self.title, description=self.type)
